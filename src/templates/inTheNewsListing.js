...
const NewsListing = (props) => {
  ...
  var allPosts = props.data.allWpInthenews.edges;
  ...
  useEffect(() => {
    let featuredCatagoryOne = allPosts.filter(item => item.node.inthenewscategories.nodes[0].slug === "category-one").slice(0, featuredSize)
    let featuredCatagoryTwo = allPosts.filter(item => item.node.inthenewscategories.nodes[0].slug === "category-two").slice(0, featuredSize)
    let featuredCatagoryThree = allPosts.filter(item => item.node.inthenewscategories.nodes[0].slug === "category-three").slice(0, featuredSize)
    ...
    let nonFeaturedCatagoryOne = allPosts.filter(item => item.node.inthenewscategories.nodes[0].slug === "category-one").slice(featuredCatagoryOne.length)
    let nonFeaturedCatagoryTwo = allPosts.filter(item => item.node.inthenewscategories.nodes[0].slug === "category-two").slice(featuredCatagoryTwo.length)
    let nonFeaturedCatagoryThree = allPosts.filter(item => item.node.inthenewscategories.nodes[0].slug === "category-three").slice(featuredCatagoryThree.length)
  ...
  const filterPosts = () => {
    ...
    if (category.categoryone !== "" || category.categorytwo !== "" || category.categorythree !== "") {
      ...
      filteredData = filteredData.filter(news =>
        news.node.inthenewscategories.nodes[0].slug === category.categoryone || news.node.inthenewscategories.nodes[0].slug === category.categorytwo || news.node.inthenewscategories.nodes[0].slug === category.categorythree
      )
    }
              ...
              <div className="row">
                ...
                {featuredPosts.map((data) => {
                  return <NewsCards
                    ...
                    category={data.node.inthenewscategories.nodes[0].name}
                  />
                })}
              </div>
              :
...
query painPage($id: String!, $langCode: ID!) {
  wpPage(id: { eq: $id }) {
    title
    content      
    id 
  ...
  allWpInthenews(filter: {locale: {id: {eq: $langCode}}}, sort: {fields: date, order: DESC}) {
    edges {
        node {
            title
            id
